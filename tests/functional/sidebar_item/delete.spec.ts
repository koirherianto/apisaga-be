...
test.group(
  'Sidebar Items Delete API | DELETE | api/projects/:projectSlug/version/:version/sidebar-items/:itemSlug',
  async (group) => {
    let testUser: TestUserResult
    let testLicense: License
    let testProject: Project
    let testVersion: Version
    let testSidebarItem: SidebarItem | null
    ...
    test('Should delete a sidebar item', async ({ client, expect }) => {
      ...
      testSidebarItem = null // Item has been deleted, no need to remove in teardown
    })
