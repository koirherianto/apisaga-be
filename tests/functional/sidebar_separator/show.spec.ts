...
test.group(
  'Sidebar Separators Show API | GET | api/projects/:projectSlug/version/:version/sidebar-separator/:separatorSlug',
  async (group) => {
    ...
    test('Should show sidebar separator with valid data', async ({ client, expect }) => {
      ...
      expect(response.body().data.name).toBe(testSidebarSeparator.name)
      expect(response.body().data.order).toBe(testSidebarSeparator.order)
      expect(response.body().data.versionId).toBe(testSidebarSeparator.versionId)
